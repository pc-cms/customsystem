...
BEGIN
  -- USD→TZS rate: latest cage rate, then fin_daily_rates, then 2600
  ...
  SELECT jsonb_build_object(
    ...
    'grand_tzs', COALESCE(SUM(
      CASE WHEN currency='TZS' THEN COALESCE(starting_float_amount,0)
           ...
           ELSE COALESCE(starting_float_amount,0) END
    ),0),
  ...
  -- Missed chips (shift closing_count.chip_miss_total; negative → more money)
  ...
  v_incomes := jsonb_build_object(
    'live_game', COALESCE((
      SELECT SUM(COALESCE(result,0)) FROM table_daily_results
      WHERE casino_id = p_casino_id AND date BETWEEN p_period_start AND p_period_end
    ),0),
    'slots', COALESCE((
    ...
    'other', COALESCE((
      SELECT SUM(COALESCE(amount,0) * COALESCE(fx_rate,1)) FROM fin_other_incomes
  ...
  -- Collections (expense category with 'collection' in name/group)
  ...
  -- Expenses (all other expense wallet tx)
  ...
  -- Wallets: ledger = starting_float + Σ signed fin_wallet_tx up to period_end
  WITH tx AS (
    SELECT wallet_id,
           SUM(
             CASE
               WHEN kind='income'     THEN COALESCE(amount_tzs,0)
               WHEN kind='expense'    THEN -COALESCE(amount_tzs,0)
               WHEN kind='adjustment' THEN COALESCE(amount_tzs,0)
               ELSE COALESCE(amount_tzs,0)
             END
           ) AS ledger_delta_tzs,
           SUM(
             CASE
               WHEN kind='income'     THEN COALESCE(amount,0)
               WHEN kind='expense'    THEN -COALESCE(amount,0)
               WHEN kind='adjustment' THEN COALESCE(amount,0)
               ELSE COALESCE(amount,0)
             END
           ) AS ledger_delta_native
    FROM fin_wallet_tx
    WHERE casino_id = p_casino_id
  ...
  phys AS (
    SELECT DISTINCT ON (wallet_type, currency)
           wallet_type, currency, physical_total, created_at
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'kind',      w.kind,
    'currency',  w.currency,
    'ledger',    COALESCE(w.starting_float_amount,0) + COALESCE(tx.ledger_delta_native,0),
    'physical',  p.physical_total
  ) ORDER BY w.sort_order, w.name), '[]'::jsonb)
  INTO v_wallets
  FROM fin_wallets w
  LEFT JOIN tx   ON tx.wallet_id = w.id
  LEFT JOIN phys p ON p.wallet_type = w.kind AND p.currency = w.currency
