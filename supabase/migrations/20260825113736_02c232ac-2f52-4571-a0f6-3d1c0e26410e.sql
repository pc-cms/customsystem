...
BEGIN
  ...
  -- Reverse sync only: manually entered Day Closing figures are mirrored back
  -- onto an ALREADY CLOSED slot shift. Day Closings are never sourced from it.
...
CREATE OR REPLACE FUNCTION public.close_business_day_with_figures(_casino_id uuid, _drop_slots numeric, _net_win numeric, _cashdesk_win numeric, _client_balance numeric, _notes text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT public.close_business_day_with_figures(
    _casino_id, _drop_slots, _net_win, _cashdesk_win, _client_balance, _notes, NULL::numeric, NULL::date
  );
$function$;