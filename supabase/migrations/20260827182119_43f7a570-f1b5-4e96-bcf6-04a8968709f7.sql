...
BEGIN
  ...
  -- ACE figures are authoritative: a zero is data, not "missing", so drop and
  -- slots_result are always overwritten with the latest received report.
  ...
  -- JP reconciliation: bring the posted JP total for the day to exactly _jp_in
  -- (including corrections down to zero). NULL means "ACE did not report JP".
