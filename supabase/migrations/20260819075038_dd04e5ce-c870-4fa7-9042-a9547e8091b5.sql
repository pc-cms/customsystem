...
DECLARE
  ...
  v_waiting int := 0;
BEGIN
  -- Business day rolls over at 07:00 EAT. Between 07:00 and 07:59 we close only
  -- casinos whose ACE closed report for yesterday has already arrived (grace
  -- window). From 08:00 the close is unconditional, as before.
  IF v_hour < 7 THEN
...
REVOKE EXECUTE ON FUNCTION public.force_close_business_day_0800() FROM PUBLIC, anon, authenticated;

-- Run every 10 minutes inside the 07:00-07:59 EAT grace window (04:xx UTC),
-- in addition to the existing unconditional 08:00 EAT run.
SELECT cron.schedule(
  'force-close-business-day-0700-grace',
  '0,10,20,30,40,50 4 * * *',
