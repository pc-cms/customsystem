...
BEGIN
  ...
  INSERT INTO public.fin_month_closures (
    ...
    v_tot_tzs, v_tot_usd, COALESCE(p_collection,'[]'::jsonb), COALESCE(p_new_float,'[]'::jsonb), p_note
  )
  ...
  -- 1) Collection: withdraw money out of each wallet (ledger movement)
  FOR r IN SELECT * FROM jsonb_array_elements(COALESCE(p_collection,'[]'::jsonb)) LOOP
    IF COALESCE((r->>'amount')::numeric,0) = 0 THEN CONTINUE; END IF;
    SELECT * INTO v_wallet FROM fin_wallets WHERE id = (r->>'wallet_id')::uuid;
    IF NOT FOUND THEN CONTINUE; END IF;
    v_fx := CASE WHEN v_wallet.currency = 'USD' THEN v_usd ELSE 1 END;

    -- idempotency: drop previous collection tx for this closure+wallet
    DELETE FROM fin_wallet_tx
     WHERE ref_table = 'fin_month_closures' AND ref_id = v_id AND wallet_id = v_wallet.id;
    ...
    INSERT INTO fin_wallet_tx (
      ...
      COALESCE((r->>'amount')::numeric,0), v_wallet.currency, v_fx,
      COALESCE((r->>'amount')::numeric,0) * v_fx,
      'fin_month_closures', v_id, v_last_day,
      format('Collection %s-%s', p_year, lpad(p_month::text,2,'0')), v_uid
    );
  END LOOP;

  -- 2) New starting float per wallet
  ...
  -- 3) Physical cash recount: after collection only the new float stays in the safe
  INSERT INTO cash_count_snapshots (
    casino_id, wallet_type, currency, denominations, physical_total,
    exchange_rate, counted_by, note, created_at
  )
  SELECT p_casino_id, w.kind, w.currency, '{}'::jsonb,
         SUM(COALESCE((nf->>'amount')::numeric,0)),
         CASE WHEN w.currency='USD' THEN v_usd ELSE 1 END,
         v_uid,
         format('Close Month %s-%s', p_year, lpad(p_month::text,2,'0')),
         v_next_first::timestamptz
  FROM jsonb_array_elements(COALESCE(p_new_float,'[]'::jsonb)) nf
  JOIN fin_wallets w ON w.id = (nf->>'wallet_id')::uuid
  WHERE w.casino_id = p_casino_id AND w.is_active
  GROUP BY w.kind, w.currency;

  RETURN v_id;
END
$$;

REVOKE ALL ON FUNCTION public.fin_close_month(uuid,int,int,jsonb,jsonb,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.fin_close_month(uuid,int,int,jsonb,jsonb,text) TO authenticated, service_role;