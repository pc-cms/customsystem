...
BEGIN
  ...
  -- `v_unpl_paid_cash` = paid rows NOT represented by an Actual Expense row:
  -- fin_unplanned_mark_paid posts wallet cash for them, so Cash Position must
  -- subtract them exactly once (rows with expense_id are already in expenses).
  ...
  --                − Actual Expenses − Paid Unplanned not in Actual
  ...
  IF v_closed THEN
    ...
    -- Manager Bonus stays Budget-based even after close.
...
BEGIN
  ...
  v_payload := jsonb_build_object(
    ...
    -- Frozen Manager Bonus is Budget-based, same rule as an open month.
    'manager_bonus', GREATEST(0, 0.05 * (v_income - v_budget)),
