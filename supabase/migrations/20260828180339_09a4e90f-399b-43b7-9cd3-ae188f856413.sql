...
declare
  ...
  v_close_den jsonb; v_open_den jsonb;
begin
  ...
  loop
    ...
    -- SLOTS banks: the cashdesk stores them on cash checks (denominations.bank).
    -- Closing = last non-opening check, opening = the check flagged is_opening.
    select cc.denominations into v_close_den
      from cage_slots_cash_counts cc
     where cc.cage_slots_shift_id = r.id
       and coalesce((cc.denominations->>'is_opening')::boolean, false) = false
     order by cc.created_at desc
     limit 1;
    select cc.denominations into v_open_den
      from cage_slots_cash_counts cc
     where cc.cage_slots_shift_id = r.id
       and coalesce((cc.denominations->>'is_opening')::boolean, false) = true
     order by cc.created_at asc
     limit 1;

    v_ch      := coalesce(v_close_den->'bank'->'channels','{}'::jsonb);
    v_open_ch := coalesce(v_open_den->'bank'->'channels','{}'::jsonb);
    if jsonb_typeof(v_ch) = 'object' and v_ch <> '{}'::jsonb then
      for k in select jsonb_object_keys(v_ch) loop
        v_in    := coalesce(nullif(v_ch->k->>'in','')::numeric, 0);
        v_out   := coalesce(nullif(v_ch->k->>'out','')::numeric, 0);
        ...
        insert into closing_wallet_inbox_rows(
          ...
          currency, orig_in, orig_out, orig_amount, final_balance, opening_balance,
          wallet_id, source_ref_table, source_ref_id)
        values (v_inbox, _casino_id, _business_date, 'slots', 'bank', v_lbl, v_cur,
                v_in, v_out, amt, v_final, v_open,
                closing_inbox_map_wallet(_casino_id,'bank',v_cur,v_lbl),
                'cage_slots_shifts', r.id);
      end loop;
    else
      -- Legacy generic bank balances (no channels) — reference only, never posted.
      for k in select jsonb_object_keys(coalesce(v_close_den->'bank','{}'::jsonb)) loop
        continue when k = 'channels';
        continue when jsonb_typeof(v_close_den->'bank'->k) <> 'number';
        v_final := coalesce(nullif(v_close_den->'bank'->>k,'')::numeric, 0);
        continue when v_final = 0;
        insert into closing_wallet_inbox_rows(
          inbox_id, casino_id, business_date, section, source_kind, label,
          currency, orig_in, orig_out, orig_amount, final_balance, wallet_id,
          source_ref_table, source_ref_id)
        values (v_inbox, _casino_id, _business_date, 'slots', 'bank',
                'Bank ' || upper(k), upper(k), 0, 0, 0, v_final, null,
                'cage_slots_shifts', r.id);
      end loop;
    end if;
