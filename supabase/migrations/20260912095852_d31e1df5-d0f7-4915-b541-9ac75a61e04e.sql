...
BEGIN
  ...
  -- CANON: a day counts as soon as its figures exist in Day Closings.
  -- The official business-day closure lock is NOT required (identical to Dashboard TV).
  closed AS (
    SELECT d.casino_id, d.business_date
      FROM public.fin_day_closing d
      JOIN ids ON ids.cid = d.casino_id
     WHERE d.business_date BETWEEN v_from AND v_to
  ),
  -- CANON (identical to Dashboard TV monthly):
  --   Slot Result = SUM per day of (cashdesk_win - players_card_balance), signed.
  dc AS (
    SELECT d.casino_id, d.business_date,
