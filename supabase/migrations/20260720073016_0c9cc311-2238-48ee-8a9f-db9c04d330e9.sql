...
BEGIN
  ...
  v_incomes := jsonb_build_object(
    ...
    -- SLOTS protection: only include months FULLY contained inside the period.
    -- Prevents a partial-period query from picking up a whole month of slot income.
    'slots', COALESCE((
      SELECT SUM(amount) FROM fin_incomes
      WHERE casino_id = p_casino_id
        AND make_date(year,month,1) >= date_trunc('month', p_period_start)::date
        AND (make_date(year,month,1) + INTERVAL '1 month - 1 day')::date <= p_period_end
    ),0),
    'other', COALESCE((
