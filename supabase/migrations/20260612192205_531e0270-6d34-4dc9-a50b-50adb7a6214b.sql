...
-- Live Game cage: read cashless IN/OUT from manual per-provider JSONB on shift row
...
BEGIN
  ...
  -- Cashless IN / OUT: cashier's manual per-provider entries on the shift row.
  -- /cashless transactions only feed the UI as a grey placeholder hint.
...
-- Slots cage: same swap. cashless_in/out come from manual per-provider fields.
...
BEGIN
  ...
  -- Cashless IN / OUT: cashier's manual per-provider entries (same model as Live Game).
  SELECT COALESCE(SUM((value)::bigint), 0)::bigint INTO v_cashless_in
    FROM jsonb_each_text(COALESCE(s.cashless_in_providers, '{}'::jsonb));
  SELECT COALESCE(SUM((value)::bigint), 0)::bigint INTO v_cashless_out
    FROM jsonb_each_text(COALESCE(s.cashless_out_providers, '{}'::jsonb));
