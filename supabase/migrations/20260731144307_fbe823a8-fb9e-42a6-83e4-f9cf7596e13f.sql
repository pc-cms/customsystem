-- 1) Per-shift table results: closing count (Close Tables wizard) wins over
--    hourly chip snapshots and over derived daily rows. Manual imports still win.
...
-- 2) Daily results population: same precedence — a closed table uses its
--    closing count, an open table falls back to the latest chip snapshot.
