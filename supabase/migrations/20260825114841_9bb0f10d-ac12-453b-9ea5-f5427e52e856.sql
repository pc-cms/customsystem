...
BEGIN
  ...
  -- Canonical: slots_result = system result (net_win). cashdesk_win is a
  -- separate ACE metric and is NEVER used as the Slots Result.
  INSERT INTO public.fin_day_closing AS d (
    ...
    v_tables_result, _net_win, _client_balance
  )
...
CREATE OR REPLACE FUNCTION public.close_business_day_with_figures(_casino_id uuid, _drop_slots numeric, _net_win numeric, _cashdesk_win numeric, _client_balance numeric, _notes text DEFAULT NULL::text, _jp_in numeric DEFAULT NULL::numeric, _business_date date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  -- Cashier slot shifts (cage_slots_shifts) are an OPERATIONAL document only:
  -- Day Closings are never sourced from them and never write back into them.

  -- Canonical: slots_result = system result (_net_win).
  -- _cashdesk_win is stored as a separate metric only.
  INSERT INTO public.fin_day_closing AS d (
    casino_id, business_date, drop_slots, net_win, cashdesk_win,
...
CREATE OR REPLACE FUNCTION public.close_business_day_with_figures(_casino_id uuid, _drop_slots numeric, _net_win numeric, _cashdesk_win numeric, _client_balance numeric, _notes text DEFAULT NULL::text, _jp_in numeric DEFAULT NULL::numeric)
 ...
 SET search_path TO 'public'
AS $function$
  SELECT public.close_business_day_with_figures(
    _casino_id, _drop_slots, _net_win, _cashdesk_win, _client_balance, _notes, _jp_in, NULL::date
  );
$function$;