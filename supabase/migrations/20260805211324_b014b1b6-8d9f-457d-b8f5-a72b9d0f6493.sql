-- 1) Baseline auto-snapshot for every active wallet whose last snapshot
--    does not reflect the posted movements.
DO $$
DECLARE
  r RECORD;
  v_actor uuid;
...
BEGIN
  FOR r IN
    WITH w AS (
      SELECT id, casino_id, name, COALESCE(starting_float_amount,0) AS sf
      FROM fin_wallets WHERE is_active = TRUE
    ),
    p AS (
      SELECT DISTINCT ON (wallet_id) wallet_id, physical_total, created_at
    ...
    tx AS (
      SELECT t.wallet_id,
             SUM(CASE WHEN t.kind IN ('expense','manual_expense','collection','change_out')
                      THEN -abs(COALESCE(t.amount,0)) ELSE COALESCE(t.amount,0) END) AS delta_all,
             MAX(t.created_by::text)::uuid AS actor
    ...
    SELECT w.id, w.sf, p.physical_total, p.created_at AS snap_at,
           COALESCE(tx.delta_all,0) AS delta_all, tx.actor
    FROM w
    LEFT JOIN p ON p.wallet_id = w.id
    LEFT JOIN tx ON tx.wallet_id = w.id
  LOOP
    -- target = starting float + all posted movements
    v_diff := (r.sf + r.delta_all) - COALESCE(r.physical_total, r.sf);
    IF r.physical_total IS NULL THEN
      v_diff := (r.sf + r.delta_all) - r.sf;
      -- ensure a snapshot exists even when the wallet never moved
      IF v_diff = 0 THEN
        v_diff := 0;
      END IF;
    END IF;

    SELECT u.id INTO v_actor FROM auth.users u WHERE u.id = r.actor;
    IF v_actor IS NULL THEN
      SELECT u.id INTO v_actor FROM auth.users u ORDER BY u.created_at LIMIT 1;
    END IF;
    IF v_actor IS NULL THEN CONTINUE; END IF;

    -- denominations booked after the last snapshot (signed)
    SELECT COALESCE(jsonb_object_agg(k, q), '{}'::jsonb) INTO v_denoms
    FROM (
      SELECT d.key AS k, SUM((d.value)::numeric *
             CASE WHEN t.kind IN ('expense','manual_expense','collection','change_out') THEN -1 ELSE 1 END) AS q
      FROM fin_wallet_tx t
      CROSS JOIN LATERAL jsonb_each_text(COALESCE(t.denominations,'{}'::jsonb)) d(key, value)
      WHERE t.wallet_id = r.id
        AND t.posted_at IS NOT NULL
        AND COALESCE(t.kind,'') <> 'adjustment'
        AND COALESCE(t.ref_table,'') <> 'cash_count'
        AND (r.snap_at IS NULL OR t.created_at > r.snap_at)
        AND d.value ~ '^-?[0-9]+(\.[0-9]+)?$'
      GROUP BY d.key
    ) s;

    IF r.physical_total IS NULL OR v_diff <> 0 THEN
      PERFORM fin_wallet_autocount(r.id, v_diff, v_denoms, v_actor,
                                   'Baseline · sync with movements');
    END IF;
  END LOOP;
END $$;

-- 2) Actual = last snapshot, no ledger fallback (only starting float for brand new wallets)
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    -- Actual = last snapshot (manual count or automatic one written after every
    -- posted movement). Brand new wallets without any snapshot use their float.
