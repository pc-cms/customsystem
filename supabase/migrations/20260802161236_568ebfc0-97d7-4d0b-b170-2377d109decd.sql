ALTER TABLE public.cash_count_snapshots ADD COLUMN IF NOT EXISTS wallet_id uuid REFERENCES public.fin_wallets(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS ccs_wallet_created ON public.cash_count_snapshots (wallet_id, created_at DESC);
...
BEGIN
  ...
  -- Day Closing is the single source of truth for both Live and Slots results.
  ...
  WHERE e.casino_id = p_casino_id
    ...
    AND (fc.group_code ILIKE '%collection%' OR fc.name ILIKE '%collection%');
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'ledger_tzs', CASE
      ...
      ELSE COALESCE(w.starting_float_amount,0) + COALESCE(tx.delta_tzs,0)
    END,
