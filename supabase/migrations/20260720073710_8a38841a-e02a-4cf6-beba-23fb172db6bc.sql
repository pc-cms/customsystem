...
BEGIN
  -- USD rate from cage shift (source of truth), fallback to daily rate, then 2600.
  ...
  -- Missed CHIPS: business rule → chips minus means MORE cash. Sign is negated so it
  -- adds to Expected when chips are short.
  ...
  -- Missed CARDS (slots): same convention — cards short → more cash in the drawer.
  ...
  v_incomes := jsonb_build_object(
    ...
    -- SLOTS monthly income is prorated by selected calendar days.
    -- Example: selected 10 days in a 31-day month => amount * 10 / 31.
    'slots', COALESCE((
      SELECT SUM(
        amount
        * (
          LEAST((make_date(year, month, 1) + INTERVAL '1 month - 1 day')::date, p_period_end)
          - GREATEST(make_date(year, month, 1), p_period_start)
          + 1
        )::numeric
        / EXTRACT(DAY FROM (make_date(year, month, 1) + INTERVAL '1 month - 1 day'))::numeric
      )
      FROM fin_incomes
      WHERE casino_id = p_casino_id
        AND make_date(year, month, 1) <= p_period_end
        AND (make_date(year, month, 1) + INTERVAL '1 month - 1 day')::date >= p_period_start
    ),0),
    'other', COALESCE((
  ...
  -- Collections are tracked separately below; exclude them from expenses_total
  -- so the front-end can subtract both without double-counting.
  ...
  WHERE fwt.casino_id = p_casino_id
    AND fwt.business_date BETWEEN p_period_start AND p_period_end
    AND fwt.kind = 'expense'
    AND NOT (COALESCE(fc.name,'') ILIKE '%collection%' OR COALESCE(fc.group_code,'') ILIKE '%collection%');

  -- Per-wallet ledger for the PERIOD ONLY (point-in-time, like cage shift).
