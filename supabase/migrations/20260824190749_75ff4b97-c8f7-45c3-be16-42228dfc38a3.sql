-- 3) Balance snapshot v3: refund removed, closed-days only, bar income, accepted-only transfers, FX fallback
...
DECLARE
  ...
  v_bar NUMERIC;
  c_commission TEXT[] := ARRAY['other','fee','commission','agent_commission']; -- 'refund' removed: legacy rows stay readable but never counted
...
BEGIN
  ...
  -- Bar Income — paid POS revenue (cash / card portions of closed tabs).
  -- POS does NOT post to wallets today (no pos_deposit wallet tx, no bar_cash wallet),
  -- so this figure is exposed here and counted ONCE in Total Income / Expected.
  SELECT COALESCE(SUM(
           CASE
  ...
  v_incomes := jsonb_build_object(
    ...
    'slots', COALESCE((SELECT SUM(COALESCE(d.slots_result,0)) FROM fin_day_closing d
      WHERE d.casino_id=p_casino_id AND d.business_date BETWEEN p_period_start AND p_period_end
        AND EXISTS (SELECT 1 FROM business_day_closures c
                     WHERE c.casino_id=d.casino_id AND c.business_date=d.business_date)),0),
  ...
  WHERE (t.from_casino_id = p_casino_id OR t.to_casino_id = p_casino_id)
    AND t.business_date BETWEEN p_period_start AND p_period_end
    AND COALESCE(t.status,'') = 'accepted';  -- cash moves only once the transfer is accepted/posted
  ...
  inc AS (
    SELECT d.business_date,
           ...
           COALESCE(d.slots_result,0) AS slots,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'net', COALESCE(inc.live_game,0)+COALESCE(inc.slots,0)+COALESCE(inc.card_balance,0)+COALESCE(oth.other,0)+COALESCE(oth.tips_bonus,0)+COALESCE(oth.movements,0)+COALESCE(oth.add_float,0)+COALESCE(oth.jp,0)+COALESCE(bar.bar_income,0)-COALESCE(exp.expenses,0)-COALESCE(exp.collections,0)
...
-- 4) Immutable correction: reversal + replacement in ONE transaction
...
BEGIN
  ...
  IF p_amount IS NULL OR p_amount = 0 THEN RAISE EXCEPTION 'Amount must not be 0'; END IF;
...
REVOKE ALL ON FUNCTION public.fin_other_income_replace(uuid, date, uuid, text, numeric, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fin_other_income_replace(uuid, date, uuid, text, numeric, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fin_other_income_replace(uuid, date, uuid, text, numeric, uuid, text) TO service_role;