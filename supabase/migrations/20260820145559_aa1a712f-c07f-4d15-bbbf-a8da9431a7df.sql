-- Optimized monthly report: single set-based pass, precomputed FX, SECURITY DEFINER
-- with explicit per-casino access filtering.
...
BEGIN
  ...
  dc_agg AS (
    SELECT casino_id,
           ...
           SUM(slots_result)  AS slots
      FROM dc GROUP BY casino_id
  ),
  cards_last AS (
    SELECT DISTINCT ON (casino_id) casino_id, cards
      FROM dc WHERE cards > 0
  ...
  per_casino AS (
    SELECT i.cid AS casino_id,
           ...
           COALESCE(cl.cards,0)      AS players_cards,
      ...
      LEFT JOIN cards_last cl ON cl.casino_id = i.cid
...
GRANT EXECUTE ON FUNCTION public.boss_monthly_report(uuid[], int, int) TO authenticated, service_role;