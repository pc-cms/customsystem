-- Live Game cage: cashless IN/OUT is LOG-ONLY and the manual mobile (cashless)
-- Balance must be fully excluded from Cash Desk Result / Shift Balance,
-- regardless of how it was persisted (totals.mobile_tzs OR totals.mobile object
-- OR opening_float.mobile / closing_count.mobile per-provider map).
...
BEGIN
  ...
  -- Manual mobile balance — try every persisted shape, sum object values if needed.
  ...
  -- Canonical Live CDR — NO miss, NO tips, NO cashless, NO manual mobile balance.
...
-- Recompute all existing Live shifts so persisted columns match the corrected formula.
