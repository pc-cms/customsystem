...
BEGIN
  d := _from;
  WHILE d <= _to LOOP
    -- Drop R: authoritative from player_day_drop_cache (peak-NEP per business day)
    SELECT COALESCE(SUM(peak), 0)::bigint
      INTO v_drop_r
      FROM public.player_day_drop_cache
     WHERE casino_id = _casino_id
       AND business_date = d;

    -- Cash-in / cash-out by business_date (not created_at window)
    SELECT
    ...
    -- Miss + tables_result: from shifts whose business day equals d
    SELECT
    ...
    -- Tips: already scoped by business_date
