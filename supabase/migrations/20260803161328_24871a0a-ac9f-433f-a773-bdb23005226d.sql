-- 1. posted flag
ALTER TABLE public.fin_wallet_tx ADD COLUMN IF NOT EXISTS posted_at timestamptz;
UPDATE public.fin_wallet_tx SET posted_at = COALESCE(posted_at, created_at) WHERE posted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_fin_wallet_tx_pending ON public.fin_wallet_tx (casino_id, business_date) WHERE posted_at IS NULL;

-- 2. office expense -> pending until the business day is closed
...
BEGIN
  IF NEW.source = 'office' AND NEW.amount > 0 AND NEW.wallet_id IS NOT NULL THEN
    ...
    -- Only post to the wallet once the business day has been closed.
    SELECT now() INTO v_posted
    FROM public.business_day_closures c
    WHERE c.casino_id = NEW.casino_id AND c.business_date >= v_bd
    LIMIT 1;
    ...
    INSERT INTO public.fin_wallet_tx (
      ...
      NEW.created_by, NEW.denominations, v_posted
    );
  END IF;
  RETURN NEW;
END
...
-- 3. post pending movements when the business day closes
CREATE OR REPLACE FUNCTION public.fin_post_pending_wallet_tx(_casino_id uuid, _business_date date)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.tg_bdc_post_wallet_tx()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  PERFORM public.fin_post_pending_wallet_tx(NEW.casino_id, NEW.business_date);
  RETURN NEW;
END $function$;

DROP TRIGGER IF EXISTS trg_bdc_post_wallet_tx ON public.business_day_closures;
CREATE TRIGGER trg_bdc_post_wallet_tx
AFTER INSERT ON public.business_day_closures
FOR EACH ROW EXECUTE FUNCTION public.tg_bdc_post_wallet_tx();

-- 4. balance snapshot: only posted movements, and skip expenses still pending
...
-- 5. physical count / resync ignore pending movements
...
BEGIN
  ...
  SELECT COALESCE(w.starting_float_amount,0) + COALESCE(SUM(
    ...
    INTO v_base
  FROM fin_wallet_tx t WHERE t.wallet_id = p_wallet_id AND t.posted_at IS NOT NULL;
  ...
  IF abs(v_variance) >= 0.01 THEN
    INSERT INTO fin_wallet_tx (
      ...
      business_date, ref_table, ref_id, note, created_by, posted_at
    ) VALUES (
