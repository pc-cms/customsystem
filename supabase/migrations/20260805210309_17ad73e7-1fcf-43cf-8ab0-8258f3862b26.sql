...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    -- Actual = last physical count. Wallets that are never counted physically
    -- (mobile money, bank) fall back to their ledger balance, not to zero.
    'actual_native', COALESCE(ph.physical_total, COALESCE(w.starting_float_amount,0) + COALESCE(tx.delta_native,0)),
    'actual_tzs', CASE
      WHEN w.currency='TZS' THEN COALESCE(ph.physical_total, COALESCE(w.starting_float_amount,0) + COALESCE(tx.delta_tzs,0))
      WHEN w.currency='USD' THEN COALESCE(ph.physical_total, COALESCE(w.starting_float_amount,0) + COALESCE(tx.delta_native,0)) * v_usd_tzs
      ELSE COALESCE(ph.physical_total, COALESCE(w.starting_float_amount,0) + COALESCE(tx.delta_native,0)) * COALESCE(NULLIF((v_rates->>w.currency),'')::numeric, 1)
    END
