...
BEGIN
  ...
  -- Simple rule: an approved expense counts once the business day is closed
  -- (office expenses count immediately). No wallet-transaction linkage required.
  WITH e AS (
  ...
  oth AS (
    SELECT business_date, SUM(COALESCE(amount,0)*COALESCE(fx_rate,1)) AS other
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'net', COALESCE(inc.live_game,0)+COALESCE(inc.slots,0)+COALESCE(oth.other,0)-COALESCE(exp.expenses,0)-COALESCE(exp.collections,0)
  ...
  WHERE COALESCE(inc.live_game,0)<>0 OR COALESCE(inc.slots,0)<>0 OR COALESCE(oth.other,0)<>0
