-- 1) Extend allowed transaction sources -------------------------------------
...
ALTER TABLE public.fin_other_incomes ADD CONSTRAINT fin_other_incomes_source_check
  CHECK (source = ANY (ARRAY[
    'investment','inter_casino_transfer','owner_topup','office','refund','bonus',
    'tips','tips_bonus','jp','fee','other','commission','agent_commission','add_float'
  ]));

-- migrate legacy owner_topup -> office (row identity, notes and audit preserved)
UPDATE public.fin_other_incomes SET source = 'office' WHERE source = 'owner_topup';

-- 2) Intercompany transfer: funding vs repayment ----------------------------
ALTER TABLE public.fin_inter_casino_transfers
  ADD COLUMN IF NOT EXISTS kind text NOT NULL DEFAULT 'funding',
  ADD COLUMN IF NOT EXISTS repays_id uuid REFERENCES public.fin_inter_casino_transfers(id);
ALTER TABLE public.fin_inter_casino_transfers DROP CONSTRAINT IF EXISTS fin_ict_kind_check;
ALTER TABLE public.fin_inter_casino_transfers ADD CONSTRAINT fin_ict_kind_check
  CHECK (kind IN ('funding','repayment'));

CREATE OR REPLACE FUNCTION public.fin_inter_casino_send(_from_wallet_id uuid, _to_casino_id uuid, _amount numeric, _business_date date, _note text DEFAULT NULL::text, _kind text DEFAULT 'funding', _repays_id uuid DEFAULT NULL)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.fin_inter_casino_transfers (
    from_casino_id, from_wallet_id, to_casino_id, amount, currency, business_date, note, created_by, kind, repays_id
  ) VALUES (
    w.casino_id, w.id, _to_casino_id, _amount, w.currency, _business_date, NULLIF(btrim(coalesce(_note,'')),''), v_uid,
    COALESCE(_kind,'funding'), _repays_id
...
-- 3) Balance snapshot: unified buckets + Basic Float + intercompany position --
...
DECLARE
  ...
  c_commission TEXT[] := ARRAY['other','refund','fee','commission','agent_commission'];
...
BEGIN
  ...
  -- Basic Float: approved opening float carried into the period + Add Float inside it
  SELECT COALESCE(SUM(COALESCE(amount,0)*COALESCE(fx_rate,1)),0) INTO v_float_open
  ...
  SELECT COALESCE(SUM(COALESCE(amount,0)*COALESCE(fx_rate,1)),0) INTO v_float_add
  FROM fin_other_incomes
  ...
  -- Intercompany position as of period end (accepted only):
  --   liability  = funding received  − repayments sent
  --   receivable = funding sent      − repayments received
  SELECT
    COALESCE(SUM(CASE
  ...
  oth AS (
    SELECT business_date,
           SUM(COALESCE(amount,0)*COALESCE(fx_rate,1)) FILTER (WHERE COALESCE(source,'') = ANY(c_commission)) AS other,
           SUM(COALESCE(amount,0)*COALESCE(fx_rate,1)) FILTER (WHERE COALESCE(source,'') = ANY(c_tips)) AS tips_bonus,
           SUM(COALESCE(amount,0)*COALESCE(fx_rate,1)) FILTER (WHERE COALESCE(source,'') = ANY(c_move)) AS movements,
           SUM(COALESCE(amount,0)*COALESCE(fx_rate,1)) FILTER (WHERE COALESCE(source,'') = 'add_float') AS add_float,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'net', COALESCE(inc.live_game,0)+COALESCE(inc.slots,0)+COALESCE(inc.card_balance,0)+COALESCE(oth.other,0)+COALESCE(oth.tips_bonus,0)+COALESCE(oth.movements,0)+COALESCE(oth.add_float,0)+COALESCE(oth.jp,0)-COALESCE(exp.expenses,0)-COALESCE(exp.collections,0)
