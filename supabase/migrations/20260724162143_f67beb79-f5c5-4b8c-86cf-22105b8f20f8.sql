...
BEGIN
  ...
  -- Collections from `expenses` (categorised under fin_categories.group_code='collections')
  ...
  -- Real expenses from `expenses` (excluding collection-type categories)
  ...
  LEFT JOIN phys ph ON ph.wallet_type = w.kind::text AND ph.currency = w.currency
