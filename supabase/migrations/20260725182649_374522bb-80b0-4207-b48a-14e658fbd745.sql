...
BEGIN
  ...
  INSERT INTO auth.users(id, email, is_sso_user, is_anonymous) VALUES
    (v_pit,   'rls-test-pit-'   || v_pit::text   || '@example.test', false, false),
    (v_cash,  'rls-test-cash-'  || v_cash::text  || '@example.test', false, false),
    (v_admin, 'rls-test-admin-' || v_admin::text || '@example.test', false, false);
  INSERT INTO public.user_roles(user_id, role) VALUES
    (v_pit,'pit'::app_role),
    (v_cash,'cashier'::app_role),
    (v_admin,'super_admin'::app_role);
  INSERT INTO public.user_casino_access(user_id, casino_id, granted_by) VALUES
    (v_pit,  v_casino_a, v_admin),
    (v_cash, v_casino_a, v_admin);
  ...
  BEGIN
    UPDATE public.dealer_attendance SET value='A'
      WHERE casino_id=v_casino_a AND employee_id=v_employee AND date=v_today AND recorded_by=v_pit;
  ...
  BEGIN
    UPDATE public.table_tracker SET value=7
      WHERE casino_id=v_casino_a AND table_id=v_table AND date=v_today AND time_slot='10:00' AND recorded_by=v_pit;
    IF NOT FOUND THEN RAISE EXCEPTION 'no rows updated'; END IF;
  ...
  RESET ROLE;
  INSERT INTO public.user_casino_access(user_id, casino_id, granted_by) VALUES (v_pit, v_casino_b, v_admin);
  PERFORM set_config('request.jwt.claim.sub', v_pit::text, true);
  SET LOCAL ROLE authenticated;
  ...
  DELETE FROM public.user_casino_access WHERE user_id IN (v_pit, v_cash, v_admin) OR granted_by IN (v_pit, v_cash, v_admin);
  DELETE FROM public.user_roles         WHERE user_id IN (v_pit, v_cash, v_admin);
  DELETE FROM public.profiles           WHERE user_id IN (v_pit, v_cash, v_admin);
