...
Да, Net Win заполняется автоматически: ACE-коллектор при закрытии периода записывает его в закрытие дня (вместе с Drop, CashDesk Win, Card Balance). Ручной ввод — только подстраховка, когда ACE не прислал данные.

Поэтому единственное место, где Net Win виден и правится вручную — **Statistics → Slots** (отчёт истории слотов). Автозапись от коллектора не трогаем.
