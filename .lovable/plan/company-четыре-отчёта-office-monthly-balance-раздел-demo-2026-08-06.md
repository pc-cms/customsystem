...
- **Casino Monthly Balance** — существующий отдельный пункт левого меню `/reports/daily-balance` (переезжает из FINANCE в COMPANY, дубля не остаётся).
...
Страница **Office и все её вкладки не трогаются** — ничего из существующего функционала не удаляется и не переносится, Company это новый, дополнительный раздел.
