...
**Аруша (август):** физический баланс кошельков (Actual) не меняется вообще — проводка остаётся. Меняется только раскладка Expected:
...
**Мванза (август):** отток 10М от 06.08 сейчас вообще не участвует в Expected (это ручная проводка по кошельку без записи расхода), хотя деньги физически ушли и Actual их уже потерял. После правки Transfers учтёт этот отток:

```text
– TRANSFERS   −10 000 000 (расход 17.08) → −20 000 000 (17.08 + трансфер 06.08)
= EXPECTED    уменьшится на 10 000 000
= VARIANCE    станет на +10 000 000 ближе к нулю
```

То есть в Мванзе это закрывает существующую дыру в 10М между Expected и фактом, а в Аруше просто переносит сумму из Other Income в Transfers. Если расход 17.08 и трансфер 06.08 — это на самом деле одна и та же отправка, скажи: тогда я не буду учитывать расход 17.08 дважды и Expected Мванзы останется прежним.
