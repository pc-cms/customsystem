...
## 6. Защита существующих балансов (делается первым)

Старые забитые смены и закрытия трогать нельзя, поэтому изменение — только в отображении и в новых записях:

- Ничего не пересчитывается задним числом: старые проводки `fin_wallet_tx` остаются как есть, миграций, меняющих суммы прошлых дней, не будет.
- Перед работой снимается снимок: баланс каждого кошелька и Variance по всем казино (сейчас 0 везде, в Аруше — обоснованный минус). После реализации сравнивается 1:1 — цифры обязаны совпасть.
- Ячейка сетки становится «сумма существующих проводок за день», а не отдельным независимым числом, поэтому итог по кошельку не меняется — меняется только то, как день разложен на экране.
- Если сравнение снимков покажет расхождение хоть в одном казино — изменение откатывается и я докладываю до продолжения.
