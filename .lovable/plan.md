...
- В квадратике дня стоит **код казино**: `ARU`, `MWZ`, `MBY`, `DOD`. Пусто = выходной. Есть также `L` (Leave).
- Каждое казино — свой цвет квадратика, чтобы месяц читался с одного взгляда.
- Смены не разделяются: менеджеры 10:00–18:00 и 18:00–06:00, CCTV всегда с 18:00 — часы считаются по группе, в сетке отображается только код казино.
- Внизу — итоги: сколько дней человек отработал и разбивка по казино (например `ARU 12 · MWZ 6`).
