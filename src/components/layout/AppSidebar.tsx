...
import {
  ...
  RefreshCw, FlaskConical, AlertTriangle, HandCoins, User as UserIcon, Rows3, Rows2, Gift, CheckCircle2, Coffee, Megaphone, TrendingUp, ArrowLeftRight, BookOpen, Activity, SlidersHorizontal,
} from "lucide-react";
...
const ATTENDANCE_SUBITEMS: VirtualSub[] = [
  ...
  // Management attendance: floor manager level and above only (Pit and HR must not see it).
...
const ROTA_SUBITEMS: VirtualSub[] = [
  ...
  // Management rota: floor manager level and above only (Pit and HR must not see it).
...
const SidebarSections = ({
  ...
  const renderVirtualGroup = (
    ...
    const subs = allSubs.filter(s => {
      if (s.roles && !isSuper && !s.roles.some(r => (roles as string[]).includes(r))) return false;
