 ...
 *
 * The heavy lifting lives in `loadLiveShiftReportData` — a plain async
 * function — so the very same figures can be frozen into an immutable
 * report snapshot at closing time instead of being recomputed on every
 * preview / reprint.
 */
...
export type LiveShiftReportData = {
  rows: LiveTableRow[];
  totalDrop: number;
  cashlessIO: { inByProv: Record<string, number>; outByProv: Record<string, number> };
  transfers: { addFloat: number; slotsOut: number };
};

export type LiveShiftReportOpts = {
...
const EMPTY: LiveShiftReportData = {
  rows: [],
  totalDrop: 0,
  cashlessIO: { inByProv: {}, outByProv: {} },
  transfers: { addFloat: 0, slotsOut: 0 },
};

/** Point-in-time load of every figure the Live Game sheet prints. */
export const loadLiveShiftReportData = async (
  opts: LiveShiftReportOpts,
): Promise<LiveShiftReportData> => {
  const { casinoId, shiftId, businessDate, tables } = opts;
  if (!casinoId || !shiftId) return EMPTY;

  const totalDrop = await fetchTotalDrop({ casinoId, fromDate: businessDate });
  ...
  const rows: LiveTableRow[] = tables
  ...
  let cashlessIO = { inByProv: baseIn, outByProv: baseOut };
  ...
  if (shiftRow?.opened_at) {
    ...
    cashlessIO = { inByProv: mergedIn, outByProv: mergedOut };
  }

  return { rows, totalDrop, cashlessIO, transfers: { addFloat, slotsOut } };
};

/**
 * Live (unfrozen) variant used for open shifts. When `frozen` is supplied the
 * hook returns it as-is and never touches the database.
 */
export const useLiveShiftReportData = (
  opts: LiveShiftReportOpts & { frozen?: LiveShiftReportData | null },
): LiveShiftReportData => {
  const { casinoId, shiftId, businessDate, tables, frozen } = opts;
  const [state, setState] = useState<LiveShiftReportData>(frozen || EMPTY);

  useEffect(() => {
    if (frozen) { setState(frozen); return; }
    let cancelled = false;
    (async () => {
      const next = await loadLiveShiftReportData({ casinoId, shiftId, businessDate, tables });
      if (!cancelled) setState(next);
    })();
    return () => { cancelled = true; };
  }, [casinoId, shiftId, businessDate, tables, frozen]);
