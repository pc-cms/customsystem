...
const CloseShiftDialog = ({
  ...
  // End Day per provider is MANUAL-ONLY. We do not hydrate from any prior
  // check or shift — empty stays empty (prints as "—"), filled stays as the
  // cashier typed. This prevents results / NET / carry-over from silently
  // overwriting the closing entry on the printed report.
  ...
  const cashByCurrencyDenoms = useMemo(
    () => CURRENCIES.map(cur => {
      const denoms = Array.from(new Set([
        ...allDenoms(cur),
        ...Object.keys(cashCounts[cur] || {}).map(Number).filter(Number.isFinite),
      ])).sort((a, b) => b - a).filter(d => (cashCounts[cur]?.[d] || 0) > 0);
