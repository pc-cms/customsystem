...
export const MODULE_LIVE_SPEC: Partial<Record<ModuleKey, LiveTableSpec[]>> = {
  ...
  pit_active_players: [
    ...
    { table: "player_day_drop_cache", queryKeyPrefixes: ["total-drop-cache", "table-results-drop-cache", "players-drop-cache-today", "players-drop-cache-range", "player-drop-cache-by-days", "players-drop-split", "player-drop-split"] },
  ],
  pit_book: [
  ...
  finance_wallets: [
    ...
    { table: "fin_wallet_tx", queryKeyPrefixes: ["fin-wallet-tx", "fin-wallets", "fin-balance-snapshot"] },
    { table: "cash_count_snapshots", queryKeyPrefixes: ["wallet-last-counts", "wallet-tx-since-count", "fin-balance-snapshot"] },
  ],
  finance_dashboard: [
    { table: "fin_wallets", queryKeyPrefixes: ["fin-wallets", "fin-balance-snapshot"] },
