...
BEGIN
  ...
  ace_cashless AS (
    SELECT s.casino_id, s.business_date,
           SUM(COALESCE(s.cashless_money_difference,0))::numeric AS cashless_diff
      FROM public.ace_finance_snapshots s
      JOIN closed cl ON cl.casino_id = s.casino_id AND cl.business_date = s.business_date
     WHERE s.period_id > 0
       AND s.apply_status = 'applied'
     GROUP BY s.casino_id, s.business_date
  ),
  dc AS (
    SELECT d.casino_id, d.business_date,
           ...
           (COALESCE(d.cashdesk_win,0)
            - COALESCE(d.players_card_balance,0)
            - COALESCE(ac.cashless_diff,0))::numeric AS slots_net
      FROM public.fin_day_closing d
      JOIN closed cl ON cl.casino_id = d.casino_id AND cl.business_date = d.business_date
      LEFT JOIN ace_cashless ac ON ac.casino_id = d.casino_id AND ac.business_date = d.business_date
  ),
  dc_agg AS (
    SELECT casino_id,
  ...
  -- Collections group expenses; CAPEX is tracked separately (Monthly Report parity).
  coll_exp AS (
    SELECT e.casino_id, e.business_date, e.amount, e.amount_tzs,
           upper(COALESCE(e.currency,'TZS')) AS cur,
           (upper(btrim(COALESCE(fc.name,''))) = 'CAPEX') AS is_capex
  ...
  coll_exp_tzs AS (
    SELECT c.casino_id, c.business_date, c.is_capex,
  ...
  -- Office -> Collections entries (fin_other_incomes, source = 'collection').
  -- Stored as negative amounts (money taken out) -> positive collection.
  coll_office AS (
    SELECT oi.casino_id, oi.business_date,
           (upper(btrim(COALESCE(fc.name,''))) = 'CAPEX') AS is_capex,
           (-oi.amount * COALESCE(oi.fx_rate,1)) AS amt
      FROM public.fin_other_incomes oi
      JOIN ids ON ids.cid = oi.casino_id
      LEFT JOIN public.fin_categories fc ON fc.id = oi.fin_category_id
     WHERE oi.business_date BETWEEN v_from AND v_to
       AND oi.reverses_id IS NULL
       AND oi.reversed_by_id IS NULL
       AND oi.source = 'collection'
  ),
  coll_all AS (
    SELECT casino_id, business_date, is_capex, amt FROM coll_exp_tzs
    UNION ALL
    SELECT casino_id, business_date, is_capex, amt FROM coll_office
  ),
  coll_agg AS (
    SELECT casino_id,
           SUM(amt) FILTER (WHERE NOT is_capex) AS collection,
           SUM(amt) FILTER (WHERE is_capex)     AS capex
      FROM coll_all GROUP BY casino_id
  ),
  daily_collection AS (
    SELECT business_date AS date, SUM(amt) AS collection
      FROM coll_all WHERE NOT is_capex GROUP BY business_date
  ),
  budget_agg AS (
    SELECT b.casino_id,
           SUM(b.planned_amount * CASE WHEN b.cur = 'TZS' THEN 1 ELSE COALESCE(f.rate, 1) END) AS estimated
      FROM budget b
      LEFT JOIN fx f ON f.casino_id = b.casino_id AND f.cur = f.cur AND f.d = v_to
